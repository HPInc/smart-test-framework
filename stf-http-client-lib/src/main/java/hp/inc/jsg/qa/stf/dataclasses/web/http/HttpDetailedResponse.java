...
import hp.inc.jsg.qa.stf.framework.misc.CalendarHelper;
import org.apache.http.HttpResponse;

import java.util.Map;
import java.util.concurrent.TimeUnit;

import static hp.inc.jsg.qa.stf.framework.httpclient.HttpContentHandler.*;
...
public class HttpDetailedResponse {
    ...
    public HttpDetailedResponse(HttpResponse response, HttpRequestMethod method, Object headers, String url, String requestBody, Stopwatch elapsedTime, Long localTimestamp) throws Exception {
        ...
        this.url = url;
        this.requestHeadersRaw = extractRequestHeadersRaw(headers);
        ...
        if (this.requestBody != null) {
            this.requestPayloadSizeBytes = extractSizeInBytes(this.requestBody, "UTF-8");
        }
        this.method = method;
        ...
        if (gotValidResponse) {
            this.responseHeadersKeysAndValuesPair = extractHeaderKeysAndValues(response.getAllHeaders());
            this.statusCode = response.getStatusLine().getStatusCode();
            this.responseHeadersRaw = extractResponseHeadersRaw(response);
            this.responseBody = extractContentFromResponseBodyStream(response);
            this.responsePayloadSizeBytes = extractSizeInBytes(this.responseBody, "UTF-8");
        }
